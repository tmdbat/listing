...
$(document).ready(function ()
{
    ...
    $(document).on('click', '.topic-tag .close',  function()
    {
        ...
        switch (data_type)
        {
            ...
            case 'tag':
                data_url = G_BASE_URL + '/supply/ajax/remove_topic_relation/';
                break;
...
function send_inbox(uid,user_name){
    $.post(G_BASE_URL + '/people/ajax/user_dialog/', {'uid': uid,}, function (result){
        if(result.errno == 1){
            if(result.rsm){
                window.location.href = 'inbox/read/'+result.rsm.id;
            }else{
                AWS.dialog('inbox', user_name); 
            }
        }
    }, 'json');
...
function renew(resource_id){
    $.post(G_BASE_URL + '/source/ajax/source_renew/', {'resource_id': resource_id}, function (result){
        if(result.errno == 1){
            AWS.dialog('resourcePayBox',{frist_money:result.rsm.frist_money,pay_html:result.rsm.html,pay_name:'资源续费',resource_id:result.rsm.resource_id});
            $('.pay_html').append(result.rsm.html);
        }else{
...
function assessPay(assess_id){
    $.post(G_BASE_URL + '/source/ajax/assess_pay/', {'assess_id': assess_id}, function (result){
        if(result.errno == 1){
            AWS.dialog('assessPayBox',{frist_money:result.rsm.frist_money,pay_html:result.rsm.html,pay_name:'测评付费',assess_id:result.rsm.assess_id});
            $('.pay_html').append(result.rsm.html);
        }else{
            layer.msg(result.err);
        }
...
function assessRemove(assess_id){
    if(confirm('确定要取消吗？')){
        $.post(G_BASE_URL + '/source/ajax/assess_remove/', {'assess_id': assess_id}, function (result){
            if(result.errno == 1){
                $('.assess-info'+assess_id).remove();
                layer.msg(result.err);
            }
        }, 'json');
    }
